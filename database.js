...
async function initializeDB() {
    try {
        ...
        if (row.count === 0) {
            ...
            const contacts1Updated = [{ contactUserID: userId2, alias: 'user2', contactUsername: defaultUsername2, publicKey: pub2.toString('hex') }];
            const newIv1 = crypto.randomBytes(16);
            const encContacts1Updated = encrypt(JSON.stringify(contacts1Updated), key1, newIv1);
            await db.run(
                `UPDATE contacts
